  ...
  dict for {k v} $iniDic {
    switch $k {
      ...
      {[MYSQLD DEFAULT]} {
        ...
        dict set tmpDic $k [::CommonUtil::replaceLines $v $dic]
      }
      {[NDB_MGMD]} -
      {[NDBD]} -
      {[MYSQLD]} {
        ...
        set tmpList [list]
        foreach ndic $nodes {
          set afterReplaced [concat [::CommonUtil::replaceLines $v $ndic] \n]
      ...
      {[API]} {
        ...
        set tmpList [list]
        foreach ndic $nodes {
          set tmpList [concat $tmpList [::CommonUtil::replaceLines $v $ndic]]
        }
        dict set tmpDic $k $tmpList
      }
    }
